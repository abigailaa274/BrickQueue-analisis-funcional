...
| Código | Regla |
|---|---|
| RG-01 | Un ticket no puede pasar a estado "En progreso" o "Cerrado" sin tener un Developer resolutor asignado. *(nueva)* |
| RG-02 | Solo el PM puede asignar o reasignar el Developer resolutor de un ticket. |
| RG-03 | El Developer resolutor solo puede avanzar el estado de los tickets que tiene asignados a él mismo. En caso de haber más de un resolutor asignado, cualquiera de ellos puede avanzar el estado del ticket. *(nueva)* |
...
| RG-05 | Un ticket puede tener más de un Developer resolutor asignado a la vez. *(nueva)* |
...
| RG-07 | Solo el Developer solicitante que creó un ticket puede modificar los siguientes datos: título, descripción, tipo. *(nueva — deriva de RF-02, escenario 2)* |
...
| RG-10 | Todo ticket se crea automáticamente en estado "Nuevo", sin posibilidad de que el solicitante elija otro estado inicial. *(nueva)* |
